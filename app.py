...
defaults = {
    ...
    "tpl_sort_panel_open": False,  # ✅ (1번) 템플릿 순서정렬 패널 접기/펼치기(기본 접힘)
}
...
if st.session_state.admin_ok:
    ...
    for i, a in enumerate(filtered, start=2):
        with tabs[i]:
            ...
            # ✅ (2번) 통장 내역 아래에 "개별 관리자 입금/출금" 추가
            st.divider()
            st.markdown("### ➕ 개별 관리자 입금/출금")

            admin_pin_ind = st.text_input("관리자 PIN", type="password", key=f"admin_pin_ind_{sid}").strip()
            ...
            labels_ind = ["(직접 입력)"] + [template_display_for_trade(t) for t in TEMPLATES]
            sel_ind = st.selectbox("내역 템플릿", labels_ind, key=tpl_key)

            prev_k = f"admin_ind_tpl_prev_{sid}"
            prev_sel = st.session_state.get(prev_k)
            if sel_ind != prev_sel:
                st.session_state[prev_k] = sel_ind
                if sel_ind != "(직접 입력)":
            ...
            st.radio(
                ...
                on_change=_apply_admin_quick
            )

            if st.button("금액 초기화", key=f"admin_ind_reset_{sid}", use_container_width=True):
                st.session_state[dep_key] = 0
                st.session_state[wd_key] = 0
            ...
            if st.button("저장(관리자)", key=f"admin_ind_save_{sid}", use_container_width=True):
                ...
                if not is_admin_pin(admin_pin_ind):
                    ...
                    res = api_admin_add_tx_by_student_id(admin_pin_ind, sid, memo, dep, wd)
                    if res.get("ok"):
                        toast("저장 완료!", icon="✅")
                        api_list_accounts_cached.clear()
                        # 개별 탭 최신값 보이게 즉시 리로드
    ...
    # =========================
    # 설정 탭 (순서: 일괄지급/벌금 → 템플릿정렬(접기) → 템플릿추가/삭제 → PIN재설정(맨 아래))
    # =========================
    with tabs[0]:
        st.subheader("⚙️ 설정")
        admin_pin = ADMIN_PIN
        ...
        # -------------------------------------------------
        # 2) ✅ (1번) 템플릿 정렬/관리 = "접기/펼치기"로 감싸고 기본 접힘
        # -------------------------------------------------
        ...
        if not st.session_state.tpl_sort_panel_open:
            ...
            if not work_ids:
                ...
                if st.session_state.tpl_mobile_sort_ui:
                    ...
                    with b1:
                        if st.button("위로 ▲", key="tpl_simple_up", disabled=(not st.session_state.tpl_sort_mode) or pick_i == 0, use_container_width=True):
                            ...
                            st.rerun()
                    with b2:
                        if st.button("아래로 ▼", key="tpl_simple_dn", disabled=(not st.session_state.tpl_sort_mode) or pick_i == (len(work_ids) - 1), use_container_width=True):
                            work_ids[pick_i + 1], work_ids[pick_i] = work_ids[pick_i], work_ids[pick_i + 1]
                            st.session_state.tpl_work_ids = work_ids
